...
#
# Modifications Copyright 2024 nociza.
#
...
flags.DEFINE_string(
    "data_dir",
    None,
    "Path to directory containing original NQ"
        ...
        for inpath in glob.glob(os.path.join(FLAGS.data_dir, "nq-*-??.jsonl.gz")):
            ...
            with gzip.open(inpath, "rb") as fin:
                for l in fin:
                    ...
                    utf8_out = (
                        json.dumps(text_utils.simplify_nq_example(json.loads(utf8_in)))
                        + "\n"
                    )
